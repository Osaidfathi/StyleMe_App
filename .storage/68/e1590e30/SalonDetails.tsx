...
const SalonDetails: React.FC = () => {
  ...
  const [selectedBarber, setSelectedBarber] = useState<typeof mockSalons[0]['barbers'][0] | null>(null);
  ...
  const handleBarberClick = (barber: typeof mockSalons[0]['barbers'][0]) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <TabsContent value="barbers" className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            {salon.barbers.map((barber) => (
              <Card key={barber.id} className="cursor-pointer hover:shadow-md transition-shadow">
                <CardContent className="p-4">
                  <div className="flex items-start space-x-4" onClick={() => handleBarberClick(barber)}>
                    ...
                    <div className="flex-1">
                      <h4 className="font-semibold text-lg hover:text-primary transition-colors">{barber.name}</h4>
      ...
      <BarberDetails 
        ...
        isOpen={isBarberDetailsOpen}
        onClose={() => {
          setIsBarberDetailsOpen(false);
